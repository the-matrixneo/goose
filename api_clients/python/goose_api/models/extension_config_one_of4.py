...
class ExtensionConfigOneOf4(BaseModel):
    """
    Frontend-provided tools that will be called through the frontend
    """ # noqa: E501
    available_tools: Optional[List[StrictStr]] = None
    bundled: Optional[StrictBool] = Field(default=None, description="Whether this extension is bundled with goose")
    instructions: Optional[StrictStr] = Field(default=None, description="Instructions for how to use these tools")
    name: StrictStr = Field(description="The name used to identify this extension")
    tools: List[Tool] = Field(description="The tools provided by the frontend")
    type: StrictStr
    __properties: ClassVar[List[str]] = ["available_tools", "bundled", "instructions", "name", "tools", "type"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['frontend']):
            raise ValueError("must be one of enum values ('frontend')")
        return value

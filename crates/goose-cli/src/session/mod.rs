...
// Re-export the classify_planner_response function from utils
pub use utils::classify_planner_response;
...
impl Session {
    ...
    async fn summarize_context_messages(
        ...
        utils::summarize_context_messages(messages, agent, message_suffix).await
    }
    ...
    pub async fn add_extension(&mut self, extension_command: String) -> Result<()> {
        ...
        let name = utils::generate_random_name(8);
    ...
    async fn handle_normal_mode_message(&mut self, content: &str) -> Result<()> {
    ...
    async fn handle_plan_mode_message(&mut self, content: &str) -> Result<()> {
    ...
    fn handle_goose_mode_setting(&self, mode: &str) -> Result<bool> {
    ...
    /// Handle plan command
    async fn handle_plan_command(&mut self, options: input::PlanCommandOptions) -> Result<()> {
    ...
    async fn handle_recipe_command(&mut self, filepath_opt: Option<String>) -> Result<()> {
    ...
    async fn handle_summarize_command(&mut self) -> Result<()> {
...
// Re-export the get_reasoner function from utils
pub use utils::get_reasoner;

...
impl OpenAiProvider {
    pub fn from_env(model: ModelConfig) -> Result<Self> {
        ...
        let base_path = config_builder.get_param("BASE_PATH", Some("v1/chat/completions"))
    ...
    async fn post(&self, payload: Value) -> Result<Value, ProviderError> {
        ...
        if let Some(org) = &self.organization {
            header_builder = header_builder.add_custom_header("OpenAI-Organization".to_string(), org.clone());
        }
        
        // Add project header if present
        if let Some(project) = &self.project {
            header_builder = header_builder.add_custom_header("OpenAI-Project".to_string(), project.clone());
        }

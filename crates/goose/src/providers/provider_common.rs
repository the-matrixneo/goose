...
impl HeaderBuilder {
    ...
    pub fn add_request_id(mut self) -> Self {
        ...
        self.custom_headers
            .insert("X-Trace-ID".to_string(), request_id);
        self
    }
    ...
    pub fn build(self) -> reqwest::header::HeaderMap {
        ...
        match self.auth_type {
            ...
            AuthType::Custom(header_name) => {
                if let Ok(name) = reqwest::header::HeaderName::from_bytes(header_name.as_bytes()) {
                    headers.insert(name, self.auth_token.parse().unwrap());
                }
            }
        }
        ...
        headers.insert(
            ...
            format!("Goose/{} (Rust)", env!("CARGO_PKG_VERSION"))
                .parse()
                .unwrap(),
        );
...
lazy_static! {
    static ref SHARED_CLIENT: Arc<Client> =
        Arc::new(create_default_client(None).expect("Failed to create shared HTTP client"));
}
...
pub async fn handle_provider_response(
    ...
    if status.is_success() {
        ...
        match status {
            ...
            StatusCode::TOO_MANY_REQUESTS => Err(ProviderError::RateLimitExceeded(error_msg)),
            ...
            s if s.is_server_error() => Err(ProviderError::ServerError(error_msg)),
            _ => Err(ProviderError::RequestFailed(error_msg)),
        }
    }
}
...
mod tests {
    ...
    async fn test_retry_with_backoff_success() {
        ...
        let result = retry_with_backoff(&config, || async { Ok::<i32, ProviderError>(42) }).await;

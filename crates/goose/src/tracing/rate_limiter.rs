...
impl RateLimitedTelemetrySender {
    pub fn new(rate_limit_ms: u64) -> Self {
        ...
        tokio::spawn(async move {
            ...
            info!("Starting rate-limited telemetry sender with {}ms delay", rate_limit_ms);
    ...
    pub fn send_span(&self, span_data: SpanData) -> Result<(), mpsc::error::SendError<TelemetryEvent>> {
        self.sender.send(TelemetryEvent::Span(span_data))
    }
    
    pub fn send_metric(&self, metric_data: MetricData) -> Result<(), mpsc::error::SendError<TelemetryEvent>> {

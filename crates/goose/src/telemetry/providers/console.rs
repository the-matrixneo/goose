...
impl super::TelemetryBackend for ConsoleProvider {
    async fn initialize(
        &mut self,
        config: &TelemetryConfig,
        ...
        tracing::info!("Console telemetry provider initialized successfully");
        Ok(())
    }
    ...
    async fn send_event(
        ...
        if !self.initialized {
            return Err("Console provider not initialized".into());
        }

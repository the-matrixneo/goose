...
mod format_task_timing {
    ...
    fn create_test_task_info_with_timing(start: Option<Instant>, end: Option<Instant>) -> TaskInfo {
        TaskInfo {
            task: Task {
...
mod format_task_output {
    ...
    fn formats_running_task_with_output() {
        let task_info =
            create_test_task_info_with_output(TaskStatus::Running, "Building project...");
    ...
    fn replaces_newlines_with_pipes() {
        let task_info =
            create_test_task_info_with_output(TaskStatus::Running, "line 1\nline 2\nline 3");
        let result = format_task_output(&task_info).unwrap();
...
mod format_task_display {
    ...
    fn create_comprehensive_task_info(
        ...
        let result = error.map(
            |msg| crate::agents::sub_recipe_execution_tool::types::TaskResult {

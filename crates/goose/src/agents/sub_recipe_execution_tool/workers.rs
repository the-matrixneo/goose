use crate::agents::sub_recipe_execution_tool::tasks::{process_task, SubagentExecutor};
...
pub fn spawn_worker(
    ...
    tokio::spawn(async move {
        worker_loop(state, worker_id, subagent_executor).await;
    })
}
...
async fn worker_loop(
    ...
    while let Some(task) = receive_task(&state).await {
        ...
        let result = process_task(&task, state.task_execution_tracker.clone(), subagent_executor.clone()).await;

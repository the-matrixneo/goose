...
pub const SEARCH_AVAILABLE_EXTENSIONS_TOOL_NAME: &str =
...
impl ExtensionManagerClient {
    ...
    async fn handle_read_resource(&self, arguments: Option<JsonObject>) -> Result<Vec<Content>, String> {
        let uri = arguments
            .as_ref()
            .ok_or("Missing arguments")?
            .get("uri")
...
impl McpClientTrait for ExtensionManagerClient {
    ...
    async fn call_tool(
        ...
        let content = match name {
            ...
            PLATFORM_READ_RESOURCE_TOOL_NAME => self.handle_read_resource(arguments).await,

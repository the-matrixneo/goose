    ...
    <>
      ...
      {clicked ? (
        ...
        <div className="goose-message-tool bg-bgApp border border-borderSubtle dark:border-gray-700 rounded-b-2xl px-4 pt-2 pb-2 flex gap-2 items-center">
          <Button variant="default" onClick={() => handleButtonClick(ALWAYS_ALLOW)}>
            Always Allow
          </Button>
          <Button
            variant="default"
            className="bg-bgProminent text-white dark:text-white"
            onClick={() => handleButtonClick(ALLOW_ONCE)}
          >
            Allow Once
          </Button>
          <Button variant="outline" onClick={() => handleButtonClick(DENY)}>
            Deny
          </Button>
        </div>
      )}

...
import {recordRecipeHash} from "./utils/recipeHash";
...
export default function App() {
    ...
    useEffect(() => {
        ...
        if (recipeConfig && !recipeWarningVisible && !pendingRecipe) {
            setTimeout(() => {
                continueInitialization().catch((error: Error) => {
                    console.error('Error in recipe initialization:', error);
                    setFatalError(`Recipe initialization failed: ${error.message || 'Unknown error'}`);
                });
            }, 100); // Small delay to ensure React is fully ready
        }
    }, [recipeWarningVisible, pendingRecipe, continueInitialization]);

    const handleRecipeConfirm = async () => {
        if (pendingRecipe) {
            console.log('Executing new recipe:', pendingRecipe);
            await recordRecipeHash(pendingRecipe);
    ...
    const handleRecipeCancel = () => {
        ...
        setRecipeWarningVisible(false);
        setPendingRecipe(null);
        // Close the current window since user cancelled recipe execution
    ...
    }, [read, setView, getExtensions, addExtension]);
    ...
    useEffect(() => {
        ...
        // If we have a recipe config, we might be waiting for a warning to be shown
        // Don't continue with initialization yet - wait for the warning event
        if (recipeConfig) {
            console.log('Recipe config detected, waiting for potential warning...');
            setView('loading');
            return;
        }
    ...
    }, [read, getExtensions, addExtension, continueInitialization]);

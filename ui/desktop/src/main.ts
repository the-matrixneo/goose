...
// Handle desktop file creation for dragged tiles
ipcMain.handle('create-desktop-shortcut', async (_event, data) => {
  try {
    const { title, description, iconPath } = data;

    if (process.platform === 'darwin') {
      // macOS: Create an .app bundle or .webloc file
      const desktopPath = path.join(app.getPath('desktop'), `${title}.webloc`);
      const weblocContent = `<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
  <key>URL</key>
  <string>goose://assistant</string>
</dict>
</plist>`;

      await fs.writeFile(desktopPath, weblocContent);
      return { success: true, path: desktopPath };
    } else if (process.platform === 'win32') {
      // Windows: Create a .lnk shortcut file
      const desktopPath = path.join(app.getPath('desktop'), `${title}.url`);
      const urlContent = `[InternetShortcut]
URL=goose://assistant
IconFile=${iconPath}
IconIndex=0`;

      await fs.writeFile(desktopPath, urlContent);
      return { success: true, path: desktopPath };
    } else {
      // Linux: Create a .desktop file
      const desktopPath = path.join(app.getPath('desktop'), `${title}.desktop`);
...
// Track floating button windows
const floatingButtons = new Map<string, BrowserWindow>();
...
ipcMain.handle('create-floating-button', async (event, data) => {
  ...
  try {
    ...
    // Calculate window size based on size parameter first
    const windowSize = size === 'large' ? 96 : size === 'medium' ? 80 : 64;

    console.log('Main process: Creating floating window at position:', { x, y });
    console.log('Main process: Screen coordinates received:', { x, y });
    console.log('Main process: Window size will be:', windowSize);

    // Get display information to ensure we're positioning correctly
    const { screen } = require('electron');
    const displays = screen.getAllDisplays();
    const primaryDisplay = screen.getPrimaryDisplay();
    console.log('Main process: Primary display bounds:', primaryDisplay.bounds);
    console.log(
      'Main process: All displays:',
      displays.map((d: any) => ({ id: d.id, bounds: d.bounds }))
    );

    // Find which display the coordinates are on
...
<body>
  ...
  <script>
    ...
    floatingWindow.loadURL(
      `data:text/html;charset=utf-8,${encodeURIComponent(floatingButtonHTML)}`
    );

    // Add comprehensive error handling and logging
    floatingWindow.webContents.on('did-finish-load', () => {
      console.log('Main process: Floating window content loaded successfully');
      console.log('Main process: Window bounds:', floatingWindow.getBounds());
      console.log('Main process: Window visible:', floatingWindow.isVisible());
      console.log('Main process: Window minimized:', floatingWindow.isMinimized());
      console.log('Main process: Window destroyed:', floatingWindow.isDestroyed());
    });

    floatingWindow.webContents.on('did-fail-load', (_event, errorCode, errorDescription) => {
      console.error('Main process: Floating window failed to load:', errorCode, errorDescription);
    });

    floatingWindow.webContents.on('console-message', (_event, level, message) => {
      console.log(`Floating window console [${level}]:`, message);
    });

    floatingWindow.on('show', () => {
      console.log('Main process: Floating window is now visible');
      console.log('Main process: Final window bounds:', floatingWindow.getBounds());
    });

    floatingWindow.on('ready-to-show', () => {
      console.log('Main process: Floating window is ready to show');
    });

    floatingWindow.on('hide', () => {
      console.log('Main process: Floating window was hidden');
    });

    floatingWindow.on('minimize', () => {
      console.log('Main process: Floating window was minimized');
    });

    // Force show the window after a short delay to ensure it's visible
    setTimeout(() => {
      if (!floatingWindow.isDestroyed()) {
        console.log('Main process: Force showing floating window after delay');
        floatingWindow.show();
        floatingWindow.focus();
        console.log('Main process: After force show - visible:', floatingWindow.isVisible());
        console.log('Main process: After force show - bounds:', floatingWindow.getBounds());
      }
    }, 100);

    // Store the floating window
    floatingButtons.set(buttonId, floatingWindow);

    // Handle window close
    floatingWindow.on('closed', () => {
      console.log('Main process: Floating window closed');
      floatingButtons.delete(buttonId);
    });

    // Handle window move for docking detection
    floatingWindow.on('moved', () => {
      const [x, y] = floatingWindow.getPosition();
      const parentBounds = parentWindow.getBounds();

      // Check if floating window is close to parent window for docking
      const dockThreshold = 50;
      const isNearParent =
        Math.abs(x - parentBounds.x) < dockThreshold &&
        Math.abs(y - parentBounds.y) < dockThreshold;

      if (isNearParent) {
        console.log('Main process: Floating window near parent, sending dock signal');
        // Send docking signal to parent window
        parentWindow.webContents.send('floating-button-dock', { buttonId });
        floatingWindow.close();
      }
    });

    return { success: true, windowId: floatingWindow.id };
  } catch (error) {
...
// Handle closing floating button
ipcMain.handle('close-floating-button', async (_event, buttonId) => {
  const floatingWindow = floatingButtons.get(buttonId);
  if (floatingWindow && !floatingWindow.isDestroyed()) {
    floatingWindow.close();
    floatingButtons.delete(buttonId);
    return { success: true };
  }
  return { success: false, error: 'Floating button not found' };
});

// Handle floating button docking check
ipcMain.on('check-floating-dock', (event, buttonId) => {
  const floatingWindow = BrowserWindow.fromWebContents(event.sender);
  if (!floatingWindow) return;

  // This is handled in the 'moved' event listener above
  // Just acknowledge the check
  console.log('Docking check for button:', buttonId);
});

// Handle focusing parent window from floating button
ipcMain.on('focus-parent-window', (_event, parentWindowId) => {
